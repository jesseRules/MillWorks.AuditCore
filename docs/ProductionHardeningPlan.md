...
- [ ] CI workflow is green on a representative PR. *(Waived for the 1.5.1 release on 2026-04-21: GitHub Actions is intentionally disabled at the repository level to avoid per-run cost during solo development. Gate remains unchecked to preserve the true state — CI has not run on a PR — but is explicitly not blocking this release. Revisit when Actions is re-enabled.)*
...
- [x] All six phases green, all acceptance tests passing. *(Completed 2026-04-21: Phase 1–6 green, Phase 6.5 soak green (100k events, chain intact), cross-cutting startup-validation green, and every local acceptance gate — including Docker-running SQL, Docker-stopped SQL `Inconclusive`, and MillWorks permissive consumer dry-run — passed. The CI-green-on-PR gate at :224 is waived for this release because GitHub Actions is intentionally disabled for cost. This tick records a conscious release waiver, not a CI-green run.)*
