...
- [ ] All six phases green, all acceptance tests passing. *(Blocked by three open gates: Docker-stopped SQL Inconclusive verification, CI workflow green on a representative PR, and the MillWorks consumer dry-run.)*
...
- [ ] Consumer dry-run: bump `/Users/jesse/RiderProjects/MillWorks/MillWorks.sln` to a local feed build of this AuditCore cycle, run the relevant MillWorks tests, confirm default permissive behavior, then flip a regulated test profile to `FailClosedForRegulated` and confirm regulated entities behave as expected. *(Partial 2026-04-21: MillWorks.sln permissive baseline completed against AuditCore 1.5.1-dryrun.1 from local feed; hardened API/package graph did not introduce new consumer breakage. Regulated-mode flip remains pending because current MillWorks entities expose no obvious [FERPA]/[PHI]/[SensitiveData] target; needs a separate consumer-side test seam or explicit carve-out.)*
