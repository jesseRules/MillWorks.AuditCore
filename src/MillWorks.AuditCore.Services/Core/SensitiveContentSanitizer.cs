...
{
    /// <summary>
    /// Suffix to append when truncating long values. Indicates that truncation occurred.
    /// </summary>
    private const string _truncationSuffix = "...[truncated]";

    /// <summary>
    /// Regex patterns to identify sensitive content. These are applied sequentially, and the output of one pattern feeds into the next.
    /// </summary>
    private static readonly Regex[] _sensitivePatterns =
    [
        // Connection strings (SQL Server, PostgreSQL, MySQL, generic)
        new(
            @"(?i)(server|data source|host|initial catalog|database|uid|pwd|password|user id|integrated security)\s*=\s*[^;""'\s]+",
        ...
        new(@"\b(?:4\d{3}|5[1-5]\d{2}|3[47]\d{2}|6(?:011|5\d{2}))[\s\-]?\d{4}[\s\-]?\d{4}[\s\-]?\d{1,4}\b",
            RegexOptions.Compiled),
        ...
        new(
            @"(?i)(?:key|constraint|violation|duplicate|conflict)[^.]*\b[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\b",
            RegexOptions.Compiled),
    ];
